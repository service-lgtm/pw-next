 ...
 * 获取实际显示状态（考虑过期时间）
 */
function getActualStatus(ticket: Ticket): { status: string; display: string } {
...
export default function MyTicketsPage() {
  ...
  const loadTickets = async (reset = false) => {
    ...
    try {
      ...
      // 修复图片URL
      const fixedResults = response.results.map((ticket: Ticket) => {
                        ...
                        <div className="flex-1">
                          ...
                          {ticket.expire_at && actualStatus.status === 'pending' && (
                            <p className={cn(
                              ...
                              {isExpired ? '已过期' : `过期时间：${new Date(ticket.expire_at).toLocaleString()}`}
                            </p>
                          )}
                        </div>
                      </div>
