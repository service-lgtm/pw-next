...
const MinesCard: React.FC<MinesCardProps> = (props) => {
    const { data, icon, key } = props;

    // 是否为生产中状态
    const isProducing = data.is_producing;
    ...
    return <ErrorBoundary>
        ...
        >
            <div className="flex gap-[10px]">
                {
                    getPixelResourceIcon(icon, {
                        iconSize: 34,
                        haveBackgroundWarper: true,
                    })
                }
                <div className="flex flex-col gap-[8px]">
                    <div className="text-[#FFFFFF] text-[14px] font-bold">{[data?.land_type_display, data?.land_id]?.filter(Boolean)?.join("-")}</div>
                    <div className="flex items-center text-[#999999] text-[12px]">
                        {/* 地址 */}
                        {!!data?.region_name && <div className="flex items-center gap-[8px] border-r-[1px] border-[#999999] pr-[6px]">
                            <Image
                                width={10}
                                height={12}
                                alt="address"
                        ...
                        <div className="text-[#999999] text-[12px] pl-[6px]">
                            {[data?.coordinate_x ?? "", data?.coordinate_y ?? ""].filter(Boolean).join('，')}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="w-[50%] flex flex-col gap-[12px] p-[15px] bg-[#212121] rounded-[6px]">
                    ...
                    {icon === PIXEL_RESOURCE_TYPES.FARMLAND
                        ...
                        : <div className="text-[#CCCCCC] text-[12px]">
                            <span className="text-[#E7E7E7] text-[15px]">{data?.remaining_reserves ?? 0}</span>/{data?.initial_reserves_display ?? 0}
                        </div>}
                </div>
                <div className="w-[50%] flex flex-col gap-[12px] p-[15px] bg-[#212121] rounded-[6px]">
                    <div className="text-[#999999] text-[12px]">
            ...
            {icon !== PIXEL_RESOURCE_TYPES.CITY && <div className="flex items-center justify-between gap-[6px]">
                <div className="text-[#999999] text-[12px]">
                    {/* TODO */}
                    可领取：{0}
                </div>
